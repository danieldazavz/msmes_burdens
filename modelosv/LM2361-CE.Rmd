...
horas_promedio <-  ce2019i %>%
  filter(h421a < 10) %>%
  mutate(horas_promedio = ifelse((h101a + h203a) != 0, (a194a + a195a) / (h101a + h203a), NA)) %>% #Eliminar división
  group_by(e03, e17) %>%
...
horas_promedio_5 <-  ce2019i %>%
  ...
  mutate(horas_promedio = ifelse((h101a + h203a) != 0, (a194a + a195a) / (h101a + h203a), NA)) %>%
  group_by(e03, e17_5) %>%
...
#################################################################################
#El problema es que no estoy multiplicando a194a + a195a por los números respectivos de trabajadores
...
#Creamos una nueva variable para el total de horas trabajadas anualmente
 ...
 ce2019i <-  ce2019i %>%
  mutate(h_anual = (to_horas_d/100) * 255) #Eliminar división
...
rm(horas_promedio, horas_promedio_4, horas_promedio_5, horas_promedio_5_nac, horas_promedio_6_nac)
...
Estadística descriptiva: productividad todas las empresas
